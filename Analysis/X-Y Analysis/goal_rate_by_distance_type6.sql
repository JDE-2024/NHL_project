...
--SUM(COUNT(type)) OVER (PARTITION BY group_distance, type) AS sum_of_shot_type,
--ROUND(COUNT(type)/SUM(COUNT(type)) OVER (PARTITION BY group_distance, type), 2) AS percentage_d
--ROUND((COUNT(types) * 100.0) / SUM(COUNT(types)) OVER (), 2) AS percentage_of_types,	
--ROUND((COUNT(types) * 100.0) / COUNT(goals), 2) AS percentage_of_goal_types,
--(ROUND((COUNT(types) * 100.0) / SUM(COUNT(types)) OVER (), 2) * ROUND((COUNT(types) * 100.0) / COUNT(goals), 2))::int AS percent_index
FROM YY
...
--WHERE event IN ('Goal') AND type IN ('Tip-In')
